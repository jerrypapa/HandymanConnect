...
<%
    ...
    if(handymen==null){
        ...
        for (int a=0;a<customerMessagingList.size();a++){
            if(names.size()==0){
                if(customerMessagingList.get(a).getSender_type().equalsIgnoreCase("customer")){
        ...
        <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
            ...
            <div>
                <ul class="nav nav-pills">
                    <li class="nav-item">
                        <a class="nav-link active" href="#admin" id="showAdmin">Admin Messages</a>
                    </li>
                    <li class="nav-item">
            ...
            <div class="container-fluid" id="admin">
                <div class="w-75" >
                    <div class="w-100 card" style="height: 500px;overflow-y: scroll;">
                        <div class="card-body">
                    <%
                        if(adminMessagingList!=null){
                            for (int i=0;i<adminMessagingList.size();i++){
                                /**/
                                if(adminMessagingList.get(i).getSender_type().equalsIgnoreCase("admin")){
                                    out.print("<div><div class='card' style='text-align: left;padding: 5px;color: red;width: fit-content;'>");
                                    out.print("<div>"+adminMessagingList.get(i).getSender_name()+"</div>");
                                    out.print("<div>"+adminMessagingList.get(i).getMessage()+"</div>");
                                    out.print("</div></div>");
                                }else if(adminMessagingList.get(i).getSender_type().equalsIgnoreCase("handyman")){
                    ...
                    <div>
                        <form action="HandymanServlet" method="post" id="message_form">
                            <input type="hidden" name="handyman_msg" value="handyman_msg"/>
                            <input type="hidden" name="handyman_id" value="<%=handymen.getHandymanid()%>"/>
            ...
            <div class="container-fluid w-75" id="customers">
                <%
                ...
                for (int i=0;i<names.size();i++){
                    %>
                    <a href="HandymanServlet?customermsg=<%=names.get(i)%>" class="list-group-item list-group-item-action"><%=names.get(i)%></a>
                    <%
                }
                %>
                </ul>
                ...
                %>
                <div class='alert alert-danger' role='alert'>You have no chats!</div>
                <%
                }
                    }
                %>
