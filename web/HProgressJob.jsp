    ...
    <div class="row">
        <nav class="col-md-2 d-none d-md-block bg-light sidebar">
            <div class="sidebar-sticky">
                <ul class="nav flex-column">
                    ...
                    <li class="nav-item">
                        <a class="nav-link active" href="HandymanServlet?ProgressJobs=ok">
                            <span data-feather="file"></span>
                            In-progress jobs
                        </a>
        ...
        <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
            <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                <h1 class="h2">In-progress Job Description</h1>
            </div>
                    ...
                    <ul class="list-group">
                        <%
                            for(int i=0;i<jobsList.size();i++){
                                job_id=jobsList.get(i).getJobId();
                        %>
                        <li class="list-group-item">
                            Job name:&nbsp;<span style="color: rgba(17,58,255,0.9)"><%=jobsList.get(i).getJobName()%></span>
                        </li>
                        <li class="list-group-item">
                            Offered by&nbsp;<span style="color: rgba(17,58,255,0.9)"><%=jobsList.get(i).getCustomerName()%></span>
                        </li>
                        <li class="list-group-item">
                            Status:&nbsp;<span style="color: rgba(17,58,255,0.9)"><%=jobsList.get(i).getStatus()%></span>
                        </li>
                        <li class="list-group-item">
                            Job description:&nbsp;<span style="color: rgba(17,58,255,0.9)"><%=jobsList.get(i).getJobDescription()%></span>
                        </li>
                        <li class="list-group-item">
                            Date placed:&nbsp;<span  style="color: rgba(17,58,255,0.9)"><%=jobsList.get(i).getDatePlaced()%></span>
                        </li>
                        <li class="list-group-item">
                            <b>Respond to this job:</b>
                            <div class="container">
                                <form action="HandymanServlet" method="get" id="responseForm">
                                    <div class="row">
                                        <input type="hidden" name="job_id" value="<%=job_id%>" />
                                        <input type="hidden" name="complete_job" value="done" />
                                        <label class="control-label col-4">By clicking complete you choose to set this job as done!</label>
                                        <div class="col-8">
...
<script>
    $(function () {
        $('#responseForm').on('submit',function (e) {
            //e.preventDefault();
            //alert(<%//=job_id%>);
            //window.open('/HandymanServlet?start_job=inprogress&job_id=<%//=job_id%>','_self');
            /*$.ajax({
                url: $(this).attr('action'),
                method: $(this).attr('method'),
                data: {'job_id':<%//=job_id%>,'start_job':'in progress'},
